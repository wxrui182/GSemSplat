...
# @MODIFIED
def gaussian_language_postprocess(out, depth_mode, conf_mode, desc_dim=None, desc_mode='norm', two_confs=False, desc_conf_mode=None, use_offsets=False, sh_degree=1):
    ...
    pts3d, conf, desc, desc_conf, offset, scales, rotations, sh, opacities, clip_features = torch.split(fmap, [3, 1, desc_dim, 1, 3, 3, 4, 3 * sh_degree, 1, 33], dim=-1)
    
    pts3d = reg_dense_depth(pts3d, mode=depth_mode)
    conf = reg_dense_conf(conf.squeeze(-1), mode=conf_mode)
    desc = reg_desc(desc, mode=desc_mode)
    desc_conf = reg_dense_conf(desc_conf.squeeze(-1), mode=desc_conf_mode)
    # scales = torch.ones_like(scales)* (-1000000)
    offset = reg_dense_offsets(offset)
    scales = reg_dense_scales(scales)
...
# @MODIFIED language feature
class GaussianLanguageHead(PixelwiseTaskWithDPT):
    """Version of the above, modified to also output Gaussian and Language parameters"""
    ...
    def __init__(self, net, has_conf=False, local_feat_dim=16, hidden_dim_factor=4., hooks_idx=None, dim_tokens=None,
        ...
        language_feature_channels = 33
        self.gaussian_language = PixelwiseTaskWithDPT(
            num_channels=language_feature_channels, feature_dim=feature_dim, last_dim=last_dim, hooks_idx=hooks_idx,
            dim_tokens=dim_tokens, depth_mode=depth_mode, postprocess=postprocess, conf_mode=conf_mode, head_type=head_type
        )

        # final_conv_layer_lang = self.gaussian_language.dpt.head[-1]
        # torch.nn.init.xavier_uniform_(
        #     final_conv_layer_lang.weight[:512, :, :, :],  
        #     gain=torch.nn.init.calculate_gain('relu')  
        # )
        # torch.nn.init.constant_(
        #     final_conv_layer_lang.bias[:512], 0.0
        # )        
    ...
    def forward(self, decout, img_shape):
        ...
        # language feature
        language_features = self.gaussian_language.dpt(decout, image_size=(img_shape[0], img_shape[1]))

        # post process 3D pts, descriptors and confidences
        # out = torch.cat([pts3d, local_features, gaussian_features], dim=1)
        out = torch.cat([pts3d, local_features, gaussian_features, language_features], dim=1)
        if self.postprocess:
            out = gaussian_language_postprocess(out,
                                   depth_mode=self.depth_mode,
                                   conf_mode=self.conf_mode,
                                   desc_dim=self.local_feat_dim,
                                   desc_mode=self.desc_mode,
...
def mast3r_head_factory(head_type, output_mode, net, has_conf=False, use_offsets=False, sh_degree=1):
    ...
    # @MODIFIED
        ...
        return GaussianHead(net, local_feat_dim=local_feat_dim, has_conf=has_conf,
                                               ...
                                               sh_degree=sh_degree)
    # @MODIFIED language feature
    elif head_type == 'gaussian_language_head' and output_mode.startswith('pts3d+gaussian+desc'):
        local_feat_dim = int(output_mode[19:])
        assert net.dec_depth > 9
        l2 = net.dec_depth
        feature_dim = 256
        ...
        ed = net.enc_embed_dim
        dd = net.dec_embed_dim
        return GaussianLanguageHead(net, local_feat_dim=local_feat_dim, has_conf=has_conf,
                                               num_channels=out_nchan + has_conf,
                                               feature_dim=feature_dim,
                                               last_dim=last_dim,
                                               hooks_idx=[0, l2 * 2 // 4, l2 * 3 // 4, l2],
