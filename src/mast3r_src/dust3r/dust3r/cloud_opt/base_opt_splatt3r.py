...
class BasePCOptimizer (nn.Module):
    ...
    def _init_from_views(self, view1, view2, pred1, pred2,
        ...
        pred1_pts = pred1['means']
        pred2_pts = pred2['means_in_other_view']
        ...
        # print("self.str_edges", self.str_edges) # ['1_0', '2_0', '2_1', '3_0', '3_1', '3_2', '4_0', '4_1', '4_2', '4_3', '5_0', '5_1', '5_2', '5_3', '5_4', '0_1', '0_2', '1_2', '0_3', '1_3', '2_3', '0_4', '1_4', '2_4', '3_4', '0_5', '1_5', '2_5', '3_5', '4_5']
        # 方差
        pred1_covariances_i = pred1['covariances']
        pred2_covariances_j = pred2['covariances']
        self.covariances_i = NoGradParamDict({ij: pred1_covariances_i[n] for n, ij in enumerate(self.str_edges)})
        self.covariances_j = NoGradParamDict({ij: pred2_covariances_j[n] for n, ij in enumerate(self.str_edges)})

        # 球谐系数
        pred1_harmonics_i = pred1['sh']
        pred2_harmonics_j = pred2['sh']
        self.harmonics_i = NoGradParamDict({ij: pred1_harmonics_i[n] for n, ij in enumerate(self.str_edges)})
        self.harmonics_j = NoGradParamDict({ij: pred2_harmonics_j[n] for n, ij in enumerate(self.str_edges)})

        # 透明度
        pred1_opacities_i = pred1['opacities']
        pred2_opacities_j = pred2['opacities']
        self.opacities_i = NoGradParamDict({ij: pred1_opacities_i[n] for n, ij in enumerate(self.str_edges)})
        self.opacities_j = NoGradParamDict({ij: pred2_opacities_j[n] for n, ij in enumerate(self.str_edges)})

        # 语言特征
        pred1_language_features_precomp_i = pred1['clip_features']
        pred2_language_features_precomp_j = pred2['clip_features']
        self.language_features_precomp_i = NoGradParamDict({ij: pred1_language_features_precomp_i[n] for n, ij in enumerate(self.str_edges)})
        self.language_features_precomp_j = NoGradParamDict({ij: pred2_language_features_precomp_j[n] for n, ij in enumerate(self.str_edges)})
        ...
        # 用于保存最后选择的gaussians的相关参数
        self.final_gaussian_covariances = None
        self.final_gaussian_harmonics = None
        self.final_gaussian_opacities = None
        self.final_gaussian_language_features_precomp = None
    ...
    def compute_global_alignment(self, init=None, niter_PnP=10, **kw):
        if init is None:
            ...
            init_fun.init_minimum_spanning_tree_splatt3r(self, niter_PnP=niter_PnP)
