...
@torch.no_grad()
def init_minimum_spanning_tree_splatt3r(self, **kw):
    """ Init all camera poses (image-wise and pairwise poses) given
        an initial set of pairwise estimations.
    """
    device = self.device
    pts3d, _, im_focals, im_poses, gaussian_covariances, gaussian_harmonics, gaussian_opacities, gaussian_language_features_precomp  = minimum_spanning_tree_splatt3r(self.imshapes, self.edges,
                                                          self.pred_i, self.pred_j, self.conf_i, self.conf_j, self.im_conf, self.covariances_i, 
                                                          self.covariances_j, self.harmonics_i, self.harmonics_j, 
                                                          self.opacities_i, self.opacities_j, 
                                                          self.language_features_precomp_i, self.language_features_precomp_j, self.min_conf_thr,
                                                          device, has_im_poses=self.has_im_poses, verbose=self.verbose,
                                                          **kw)
    self.final_gaussian_covariances = gaussian_covariances
    self.final_gaussian_harmonics = gaussian_harmonics
    self.final_gaussian_opacities = gaussian_opacities
    self.final_gaussian_language_features_precomp = gaussian_language_features_precomp
    # print('modified done!')
    return init_from_pts3d(self, pts3d, im_focals, im_poses)
...
def init_from_pts3d_splatt3r(self, pts3d, im_focals, im_poses):
    # init poses
    nkp, known_poses_msk, known_poses = get_known_poses(self)
    if nkp == 1:
        raise NotImplementedError("Would be simpler to just align everything afterwards on the single known pose")
...
def minimum_spanning_tree_splatt3r(imshapes, edges, pred_i, pred_j, conf_i, conf_j, im_conf, covariances_i, covariances_j, harmonics_i, harmonics_j, opacities_i, opacities_j, language_features_precomp_i, language_features_precomp_j, min_conf_thr,
                          device, has_im_poses=True, niter_PnP=10, verbose=True):
    n_imgs = len(imshapes)
    sparse_graph = -dict_to_sparse_graph(compute_edge_scores(map(i_j_ij, edges), conf_i, conf_j))
    msp = sp.csgraph.minimum_spanning_tree(sparse_graph).tocoo()
    ...
    gaussian_covariances = [None] * n_imgs
    gaussian_harmonics = [None] * n_imgs
    gaussian_opacities = [None] * n_imgs
    gaussian_language_features_precomp = [None] * n_imgs
    ...
    gaussian_covariances[i] = covariances_i[i_j].clone()
    gaussian_covariances[j] = covariances_j[i_j].clone()

    gaussian_harmonics[i] = harmonics_i[i_j].clone()
    gaussian_harmonics[j] = harmonics_j[i_j].clone()

    gaussian_opacities[i] = opacities_i[i_j].clone()
    gaussian_opacities[j] = opacities_j[i_j].clone()

    gaussian_language_features_precomp[i] = language_features_precomp_i[i_j].clone()
    gaussian_language_features_precomp[j] = language_features_precomp_j[i_j].clone()
    ...
    return pts3d, msp_edges, im_focals, im_poses, gaussian_covariances, gaussian_harmonics, gaussian_opacities, gaussian_language_features_precomp
