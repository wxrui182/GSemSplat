...
def render_cuda(
    ...
    gaussian_opacities,
    gaussian_language_feature_precomp,
    ...
    for i in range(b):
        ...
        image, language_feature_image, radii = rasterizer(
            means3D=gaussian_means[i],
            means2D=mean_gradients,
            shs=shs[i] if use_sh else None,
            colors_precomp=None if use_sh else shs[i, :, 0, :],
            language_feature_precomp=gaussian_language_feature_precomp[i],
            opacities=gaussian_opacities[i, ..., None],
            cov3D_precomp=gaussian_covariances[i, :, row, col],
        )
        all_images.append(image)
        all_radii.append(radii)
        all_language_feature_images.append(language_feature_image)

    return torch.stack(all_images), torch.stack(all_language_feature_images)
